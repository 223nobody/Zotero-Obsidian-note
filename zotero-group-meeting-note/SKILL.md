...
- For one-paper final delivery, prefer `scripts/final_gate_runner.py` after drafting/review. It runs validation, quality, domain, asset, unmatched-asset, and final-delivery checks into one `final-gate.json`, records input hashes and child command failures, and can update the sidecar. Use this runner only for one paper at a time.
...
- In controlled batch mode, run `scripts/validate_sidecar.py <sidecar.json>` before counting a sidecar as final-delivery complete. A sidecar whose `final_delivery` stage is complete while any required gate report is missing, non-pass, stale against recorded file or asset-directory input hashes, or inconsistent with the same-paper/same-note `final-gate.json` is invalid.
